...
void dfs(int node, ivv &children, bv &is_chosen){
    bool flag = false;
    for(auto new_node: children.at(node)){
        dfs(new_node, children, is_chosen);
        flag |= is_chosen.at(new_node);
    }
    is_chosen.at(node) = !flag;
}
...
int main(){
    ...
    while(!q.empty()){
        ...
        for(auto new_node: g.at(node)){
            ...
            checked.at(new_node) = true;
            q.push(new_node);
        }
    }

    bv is_chosen(n, false);
    dfs(0, children, is_chosen);
    ll ans = 0;
    for(auto b: is_chosen){
        if(b) ans++;
    }
    cout << ans << endl;
}
