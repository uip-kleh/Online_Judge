...
int main(){
    ...
    int farest_node, farest_length = 0;
    ...
    while(!q.empty()){
        ...
        for(int new_node: g.at(node)){
            ...
            if(farest_length < checked[new_node]){
                farest_node = new_node;
                farest_length = checked[new_node];
            }
            q.push(new_node);
        }
    }
    q.push(farest_node);
    checked.clear(); checked[farest_node] = 0;
    while(!q.empty()){
        int node=q.front(); q.pop();
        for(int new_node: g.at(node)){
            if(checked.find(new_node) != checked.end()) continue;
